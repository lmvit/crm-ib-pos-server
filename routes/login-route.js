...
router.post('/get-mobile-number',verifyToken,async(req,res)=>{
    try {
        const posId = req.body.user.pos_id;
        const result = await(await Database.DB.query(`select mobile_number from pos_details where pos_id = '${posId}'`)).rows;
        result ? res.status(200).send({message:'mobile exists',number : result}).end() : res.status(403).send({message:'mobile not exists'}).end();
    } catch (error) {
        console.log(error);
    }
})
